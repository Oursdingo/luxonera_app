...
export default function ProductCard({
  ...
  return (
    ...
    >
      {/* Conteneur principal */}
      <article className="bg-white rounded-xl shadow-sm md:hover:shadow-2xl overflow-hidden transition-all duration-500">
        {/* === Image principale === */}
        <div className="relative w-full h-[380px] overflow-hidden bg-neutral-50">
          {/* Image par défaut */}
          <div
            className={`absolute inset-0 transition-opacity duration-500 ${
              isDesktop && isHovered ? "opacity-0" : "opacity-100"
            }`}
          >
            <Image
              src={mainImage}
        ...
        <div className="p-5 flex flex-col gap-2">
          ...
          <h3 className="font-display text-lg sm:text-xl leading-tight md:group-hover:text-accent-gold transition-colors line-clamp-2">
            {watch.name}
          </h3>
